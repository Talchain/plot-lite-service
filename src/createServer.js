...
export async function createServer(opts = {}) {
    ...
    app.addHook('onSend', async (req, reply, payload) => {
        try {
            const hdrs = req.headers || {};
            const fromHeader = hdrs['x-request-id'] || hdrs['X-Request-Id'];
            const id = fromHeader ? String(fromHeader) : String(req.id);
            if (id) {
    ...
    app.addHook('onResponse', async (req, reply) => {
        ...
        const rawRoute = req?.routeOptions?.url ?? req.url ?? '';
    ...
    app.route({
        ...
        async handler(req, reply) {
            ...
            if (!template) {
                ...
                const payload = errorResponse('BAD_INPUT', ERR_MSG.BAD_QUERY_PARAMS);
                payload.error.fields = { template: 'template is required' };
            ...
            if (!Number.isInteger(seedNum)) {
                ...
                const payload = errorResponse('BAD_INPUT', ERR_MSG.BAD_QUERY_PARAMS);
                payload.error.fields = { seed: 'seed must be an integer' };
                return reply.code(400).send(payload);
            }
    ...
    app.setErrorHandler(async (err, req, reply) => {
        ...
        const rawRoute = (req?.routeOptions?.url ?? req?.url ?? 'unknown');
        const route = String(rawRoute).split('?')[0].split('#')[0];
        if (err?.validation) {
            try {
            ...
            try {
                ...
                const payload = formatValidationErrors((err.validation) || []);
        ...
        if (code === 'FST_ERR_CTP_INVALID_JSON_BODY' || code === 'FST_ERR_CTP_EMPTY_JSON_BODY') {
            ...
            try {
                payload.error = { type: 'BAD_INPUT', message };
            }
            catch { }
