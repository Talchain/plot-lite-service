...
let lastRequestAtISO: string | undefined = new Date().toISOString();
...
let lastConfigReloadISO: string | undefined;
...
export function recordEngineComputeMs(ms: number): void {
  ...
  if (engineSamples.length > MAX_SAMPLES) engineSamples.shift();
...
export function getLastRequestAt(): string | undefined {
  return lastRequestAtISO;
}

export function setLastConfigReloadISO(iso: string): void {
  lastConfigReloadISO = iso;
}

export function getLastConfigReloadISO(): string | undefined {
  return lastConfigReloadISO;
}

// --- P1: 429 counters ---

export function incJson429Count(): void {
  json429Count++;
}

export function incSse429Count(): void {
  sse429Count++;
}

export function getJson429Count(): number {
  return json429Count;
}

export function getSse429Count(): number {
  return sse429Count;
}

// --- P1: SSE guardrail counters ---

export function incSseOpen(): void {
  sseOpen++;
}

export function incSseClosed(): void {
  sseClosed++;
}

export function incSseTimeout(): void {
  sseTimeout++;
}

export function getSseOpen(): number {
  return sseOpen;
}

export function getSseClosed(): number {
  return sseClosed;
}
...
export function incStreamRateLimited(): void {
  streamRateLimited++;
}

export function incStreamDisconnect(): void {
  streamDisconnect++;
}

export function incStreamWriteBackpressure(): void {
  streamWriteBackpressure++;
}

export function getStreamHealthExtras(): Record<string, number> {
