...
export interface CacheInterface {
  ...
  set(key: string, value: string, ttlMs: number, tags: string[]): Promise<void>;
...
export interface CacheManager {
  get(key: string): Promise<string | null>;
  set(key: string, value: string, ttlMs: number, tags: string[]): Promise<void>;
  getOrCompute(key: string, ttlMs: number, tags: string[], computeFn: () => Promise<string>): Promise<{ value: string; fromCache: boolean }>;
  invalidateByTag(tag: string): Promise<void>;
  clear(): Promise<void>;
  stats(): { hits: number; misses: number; size: number; l2Enabled: boolean };
}
...
class MemoryCache implements CacheInterface {
  ...
  constructor(maxKeys: number = Number(process.env.CACHE_L1_MAX_KEYS || 1000)) {
  ...
  async set(key: string, value: string, ttlMs: number, tags: string[]): Promise<void> {
...
class TieredCache implements CacheInterface {
  ...
  private l2: CacheInterface | null;

  constructor(l2?: CacheInterface) {
...
class CacheManagerImpl implements CacheManager {
  ...
  private singleflight: Map<string, SingleflightEntry> = new Map();
  private l2Enabled: boolean;

  constructor(cache: CacheInterface, l2Enabled: boolean) {
    this.cache = cache;
  ...
  async getOrCompute(key: string, ttlMs: number, tags: string[], computeFn: () => Promise<string>): Promise<{ value: string; fromCache: boolean }> {
    // Check cache first
...
export function getCache(): CacheInterface | null {
