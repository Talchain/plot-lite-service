...
function getEffectiveRpm(): number {
  ...
  if (envRpm !== undefined && envRpm !== '') {
    const val = Number(envRpm);
    if (Number.isFinite(val) && val >= 0) return val;
  }
  try {
    const runtimeRpm = getRateLimitRpm();
    if (Number.isFinite(runtimeRpm) && runtimeRpm > 0) return runtimeRpm;
  } catch {}
...
export function makeRateLimiter() {
  ...
  return function rateLimiter(req: FastifyRequest, reply: FastifyReply, done: HookHandlerDoneFunction) {
    ...
    // Skip rate limiting for health/metrics/limits routes
    const url = req.url || '';
    if (url.startsWith('/v1/health') || url.startsWith('/health') || 
        url.startsWith('/metrics') || url.startsWith('/v1/limits')) {
      return done();
    }
    
    const rpm = getEffectiveRpm();
    if (rpm === 0) return done(); // RPM=0 disables rate limiting
    ...
    if (rec.count > rpm) {
      ...
      reply.header('X-RateLimit-Reset', String(Math.floor(rec.resetAt / 1000)));
