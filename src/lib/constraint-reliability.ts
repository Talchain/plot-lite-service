...
export function detectUnreliableConstraintTargets(
   ...
   * ISL emits it for every non-root target without a PU, which is exactly the
   * shape the level plan serves once the PU injector stops pinning it. But the
...
export interface AnchorOptionLike {
...
/**
 * THE ONE PREDICATE for the `observed_baseline_level` anchor. It is shared,
 * deliberately, by the sample-frame gate below AND by the constraint-PU
 * injector (`integrations/isl/constraint-pu-injection.ts`, which must NOT pin
 * such a target) — so "PLoT says this is scoreable" and "PLoT leaves ISL able
 * to score it" can never be two rules that drift.
 *
 ...
 *   5. no ParameterUncertainty on the target — ISL refuses
 *      (`target_parameter_uncertainty_shifts_base`). This module cannot see the
 *      ISL request's PU list; the injector consults THIS predicate so PLoT
 *      itself never adds one (the reason the injector imports it). A factor PU
 *      the translator emits for a `factor`-kind target is NOT suppressed: ISL
 *      then refuses the whole constraint block, which is a refusal, never a
 *      wrong number.
