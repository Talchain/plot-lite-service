...
describe('Error taxonomy proofs', () => {
  ...
  it('TIMEOUT simulated via header (dev-time)', async () => {
    const res = await fetch(`${BASE}/draft-flows?force_error=TIMEOUT`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
  ...
  it('RETRYABLE simulated via header (dev-time)', async () => {
    const res = await fetch(`${BASE}/draft-flows?force_error=RETRYABLE`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
  ...
  it('INTERNAL simulated via header (dev-time)', async () => {
    const res = await fetch(`${BASE}/draft-flows?force_error=INTERNAL`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
