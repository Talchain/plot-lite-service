...
Review repair: resolve the target and frame as a bound pair before the two paths diverge. When present root and goal-node targets disagree, retain the selected number without borrowing the node's frame, disclose `GOAL_THRESHOLD_ATTESTATION_MISMATCH` in the existing repair metadata, and leave explicit constraints intact. ISL's existing missing-frame contract withholds goal probability; PLoT does not invent another frame or silently substitute the node target. Matching root/node targets and legitimate root-only/node-only targets retain their attestation. The mismatch applies to auto-synthesis as well as explicit constraints.
