...
function writeSse(res: http.ServerResponse, id: string, event: string, data: any) {
  res.write(`id: ${id}\n`);
...
const state = new Map<string, { index: number; blipped?: boolean }>();
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (u.pathname === '/stream/cancel' && req.method === 'POST') {
      ...
      req.on('end', () => {
        const id = (() => { try { return String(JSON.parse(body)?.id || u.searchParams.get('id') || ''); } catch { return String(u.searchParams.get('id') || ''); } })();
    ...
    if (u.pathname === '/stream') {
      ...
      for (let i = st.index; i < seq.length; i++) {
        ...
        if (blip && !st.blipped && e.ev === 'token') {
          st.blipped = true; state.set(id, st);
          try { res.end(); } catch {}
          return;
        }
      }
