...
async function runCheck(
  name: string,
  run: (requestId: string) => Promise<{ requestId?: string; note?: string } | void>
): Promise<{ name: string; ok: boolean; requestId: string; note?: string }> {
...
function logResult(result: { name: string; ok: boolean; requestId: string; note?: string }) {
  const status = result.ok ? 'PASS' : 'FAIL';
  const suffix = result.note ? ` - ${result.note}` : '';
  console.log(`[${status}] ${result.name} (request_id=${result.requestId})${suffix}`);
}

function responseRequestId(res: Response, fallback: string) {
...
async function main() {
  ...
  const results = [] as Array<{ name: string; ok: boolean; requestId: string; message?: string }>;
  ...
  results.push(await runCheck('PLOT /v1/cee/draft-graph?schema=v3 returns graph', async (requestId) => {
    ...
    const nodesOk = Array.isArray((payload as any).nodes) && (payload as any).nodes.length > 0;
    const edgesOk = Array.isArray((payload as any).edges) && (payload as any).edges.length > 0;
    const analysisReadyOk = (payload as any).analysis_ready && typeof (payload as any).analysis_ready === 'object';
    const optionsOk = Array.isArray((payload as any).options);
