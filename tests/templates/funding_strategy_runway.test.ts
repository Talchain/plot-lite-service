...
describe('Template: funding_strategy_runway', () => {
  let server: any;
  let baseUrl: string;
  ...
  it('Test 1: Shape & v1.2 fields', async () => {
    const res = await fetch(`${baseUrl}/v1/templates/funding_strategy_runway/graph`);
  ...
  it('Test 2: Determinism', async () => {
    const graphRes = await fetch(`${baseUrl}/v1/templates/funding_strategy_runway/graph`);
