...
describe('Template: experiment_vs_decide_now', () => {
  let server: any;
  let baseUrl: string;
  ...
  it('Test 1: Shape & v1.2 fields', async () => {
    const res = await fetch(`${baseUrl}/v1/templates/experiment_vs_decide_now/graph`);
  ...
  it('Test 2: Determinism', async () => {
    const graphRes = await fetch(`${baseUrl}/v1/templates/experiment_vs_decide_now/graph`);
