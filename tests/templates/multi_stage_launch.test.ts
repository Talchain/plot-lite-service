...
describe('Template: multi_stage_launch', () => {
  let server: any;
  let baseUrl: string;
  ...
  it('Test 1: Shape & v1.2 fields', async () => {
    const res = await fetch(`${baseUrl}/v1/templates/multi_stage_launch/graph`);
  ...
  it('Test 2: Determinism', async () => {
    const graphRes = await fetch(`${baseUrl}/v1/templates/multi_stage_launch/graph`);
