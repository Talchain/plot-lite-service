...
describe('Nested additionalProperties: /v1/counterfactual', () => {
  it('rejects unknown nested key intervention.extra with 400 BAD_INPUT', async () => {
    const url = `http://127.0.0.1:${port}/v1/counterfactual`;
    const body = {
      graph: { nodes: [{ id: 'a', label: 'A' }], edges: [] },
      intervention: { node_id: 'a', from_value: 0, to_value: 1, extra: 123 },
      outcome_node: 'a',
    };
